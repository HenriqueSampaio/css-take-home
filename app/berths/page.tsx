  ...
  return (
    <div className="mx-auto flex max-w-[64rem] flex-col gap-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="t-display">Berths</h1>
            ...
            <li key={berth.id} className="row-hover flex flex-col gap-3 px-5 py-4" style={{ ["--i" as string]: Math.min(i, 8) }}>
              <div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-2">
                <div className="min-w-0 flex-1 basis-64">
                  ...
                  <div className="fitbar mt-2 max-w-sm" aria-hidden><span style={{ width: `${Math.max(3, (berth.lengthFt / maxFt) * 100)}%`, background: "var(--color-brand)" }} /></div>
                </div>
                ...
                <div className="flex flex-col items-start gap-1 sm:items-end sm:text-right">
                  {berth.current && CurrentIcon ? (
                    <p className="flex flex-wrap items-center gap-1.5 text-[0.875rem]">
                      <span className="pill pill-brand"><CurrentIcon size={12} />In use today</span>
                    ...
                    <p><span className="pill pill-ok">Free today</span></p>
                  )}
                  {berth.next && (
                    <p className="t-small text-ink-2">Next: <Link className="link" href={scheduleHref(yearMonthOf(berth.next.startDate), berth.next.id)}>{berth.next.label}</Link> on <strong className="t-num">{formatDate(berth.next.startDate, "short")}</strong></p>
                  )}
                  {!berth.current && !berth.next && <p className="t-small text-ink-3">Nothing booked ahead</p>}
                </div>
              </div>
