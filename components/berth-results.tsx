    ...
    <div className="flex flex-col gap-4">
      ...
      {blocked.length > 0 && (
        <details className="group" open={usable.length === 0}>
          <summary className="flex cursor-pointer list-none items-center gap-1.5 py-1 text-[0.875rem] font-semibold text-ink-2 hover:text-ink [&::-webkit-details-marker]:hidden">
          ...
          <ul className="sheet rows cascade mt-2 overflow-hidden">
    ...
    <li className="grid gap-x-6 gap-y-3 px-5 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center" style={{ ["--i" as string]: Math.min(index, 6) }}>
      <div className="flex min-w-0 flex-col gap-2">
        <p className="flex flex-wrap items-center gap-x-2.5 gap-y-1">
          ...
          <span className="t-data t-num text-ink-3">{option.berth.lengthFt} ft</span>
          {option.verdict === "available" && <span className="tag tag-clear"><Check size={12} />{option.fit ? "Fits and free" : "Free"}</span>}
          {option.verdict === "too_short" && <span className="tag tag-danger"><Ruler size={12} />Too short</span>}
          {option.verdict === "occupied" && <span className="tag tag-danger"><Cross size={12} />Occupied</span>}
          {picked && <span className="tag tag-accent">You picked this one</span>}
        </p>

        {option.fit && (
          <div className="flex max-w-md flex-col gap-1.5">
            <FitGauge fit={option.fit} scaleFt={scaleFt} />
