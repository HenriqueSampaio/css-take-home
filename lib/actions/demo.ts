...
/**
 * Puts the shared demo back to its starting state: the seeded berths and vessel
 * registry, and an empty schedule. Takes no arguments on purpose: this is a
 * public endpoint, so the caller gets no say over which seed is loaded or over
 * the cooldown that stops it being hammered.
 */
